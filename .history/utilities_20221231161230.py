...
def restriction(Nx, Ny, phic, Rc_new, aEc, aWc, aNc, aSc, a0c, x_list1, y_list, x_list2, y_list2):
    phic = np.zeros((Nx, Ny))
    
    # Smoothing the errors on the coarse mesh
    # Use the correction form [A'][phi'] = [R] to calculate the correction form of phi
 
    phic = GS(Nx, Ny, phic, Rc_new, aEc, aWc, aNc, aSc, a0c)
    # phic += corrector
    R2c, _, R = residual(Nx, Ny, phic, Rc_new, aEc, aWc, aNc, aSc, a0c, convert=False)
