...
def restriction(Nx, Ny, phi, R_new, aEc, aWc, aNc, aSc, a0c, x_list1, y_list, x_list2, y_list2):
    phi = p.zeros((Nx, Ny))
    
    # Smoothing the errors on the coarse mesh
    # Use the correction form [A'][phi'] = [R] to calculate the correction form of phi
 
    phi = S(Nx, Ny, phi, R_new, aEc, aWc, aNc, aSc, a0c)
    # phi +=corrector
    R2c, _, R = residual(Nx, Ny, phi, R_new, aEc, aWc, aNc, aSc, a0c, convert=False)
